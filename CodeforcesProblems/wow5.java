...
class ActualSolution implements Runnable {
    ...
    void solve() {
        ...
        boolean startAlso = false;
 
        for (int i = 0; i < m; i++) {
            int l = sc.nextInt();
            int r = sc.nextInt();
 
            int ones = count[r] - count[l - 1];
            if (ones == 0 || ones == r - l + 1) {
                startAlso = true;
                continue;
            }
 
            int start = UpperBound(count, count[l - 1]);
            int end = search(count, l, r + 1, ones + count[l - 1]) - 1;
            debug(l, r, start, end, ones);
    ...
    int search(int[] a, int l, int r, int ones) {
        int temp = r;
        while (l < r) {
            int m = (l + r) >>> 1;
            if (ones - a[m - 1] >= temp - m) r = m;
            else l = m + 1;
        }
        return r;
    }
